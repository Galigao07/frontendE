...
interface  GIftCheckPaymentData{
    handleClose:() => void;
    amountdue:any;
    amounttendered:any;
}

const GIftCheckPaymentEntryModal: React.FC<GIftCheckPaymentData> = ({handleClose,amountdue,amounttendered}) => {
...
const TotalGiftCheck = () => {
  const x = localStorage.getItem('GiftCheckPayment');
    let total:any = 0
    let count:any = 0
    let isIncome :any = false
    let excessAmt:any = 0
  if (x) {
    const arr = JSON.parse(x); // ✅ parse string → array
 
     total = arr.GiftCheckPaymentList.reduce((sum: number, item: any) => {
      // 🧠 choose what to sum (example: item.amount)
      return sum + (Number(item.amount) || 0);
    }, 0);

      count = arr.GiftCheckPaymentList.reduce((sum: number, item: any) => {
      // 🧠 choose what to sum (example: item.amount)
      return sum + (Number(item.count) || 0);
    }, 0);

         if (arr.GiftCheckPaymentList.length > 0 ) {
             isIncome = arr.GiftCheckPaymentList[0].isIncome;
            
    }
  }
        excessAmt = total - amountdue
      return { total: Number(total) || 0, 
    count: Number(count) || 0,isIncome,excessAmt};
};
    ...
    const handleOk = () => {
        ...
        if (Number(TotalGiftCheck().excessAmt) < 0 ) {
        Swal.fire({
          title: 'Error',
      ...
      <div style={{display:'flex',flexDirection:'row' ,width:'120%' }}>
          <div style={{display:'flex',flexDirection:'column' , border:' 2px solid #ccc', borderRadius: '8px',padding:'20px'}}>
                  <p style={{ fontSize: '20px', fontWeight: 'bold',textShadow: '2px 2px 4px rgba(0, 0, 0, 0.3)'}}>TOTAL AMOUNT DUE:</p>
                    ...
                    <p style={{ fontSize: '20px', fontWeight: 'bold',textShadow: '2px 2px 4px rgba(0, 0, 0, 0.3)'}}>GIFT CHECK AMOUNT</p>
                      ...
                      id="input1"
                      readOnly
                      value={Number(TotalGiftCheck()?.total || 0).toLocaleString(undefined,{
                  ...
                  <p style={{ fontSize: '20px', fontWeight: 'bold',textShadow: '2px 2px 4px rgba(0, 0, 0, 0.3)'}}>AMOUNT TENDERED</p>
                    ...
                    <p style={{ fontSize: '20px', fontWeight: 'bold',textShadow: '2px 2px 4px rgba(0, 0, 0, 0.3)'}}>GIFT CHECK AMOUNT EXCESS</p>
                      <input style={{ fontSize: '20px', fontWeight: 'bold', border: '1px solid', margin: '10px',  color: 'Black', 
                      padding: '8px', textShadow: '2px 2px 4px rgba(0, 0, 0, 0.3)',  borderRadius: '5px'  ,textAlign:'end' ,width:'93%'}}
                      type="text"
                      autoComplete="off"
                      ...
                      readOnly
                     value={(
                            Number(TotalGiftCheck().excessAmt || '0.00')
                      ...
                      <div style={{display:'flex',flexDirection:'row',justifyContent:'center',gap:'10px'}}>
                            <input type='Checkbox' checked={TotalGiftCheck().isIncome}/>
                            <label>Excess Amount as Income</label>
                      </div>
...
export default GIftCheckPaymentEntryModal;